...
compilePackage() {
  ...
  echo "======           Create ${1}/index.d.ts re-export file for Closure"
  echo "$(cat ${LICENSE_BANNER})${N} export * from './public_api'" > ${2}/index.d.ts
  echo "{\"__symbolic\":\"module\",\"version\":3,\"metadata\":{},\"exports\":[{\"from\":\"./${package_name}/index\"}],\"flatModuleIndexRedirect\":true}" > ${2}/index.metadata.json
...
NGC="node --max-old-space-size=3000 `pwd`/node_modules/@angular/tsc-wrapped/src/main"
...
do
  ...
  if [[ ${BUNDLE} == true ]]; then
    ...
    (
        cd  ${SRC_DIR}
        echo "======         Rollup ${PACKAGE}"
        rollupIndex ${OUT_DIR} ${MODULES_DIR} "${ROOT_DIR}/${PACKAGE}/rollup.config.js"
